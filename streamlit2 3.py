...
# ------------------- Pagina 2 --------------------------
    ...
    def parse_duration_to_hours(value):
        ...
        if value == "an hour":
            return 1.0
        match = re.search(r"(\d+)\s*hour", value)
        if match:
            return float(match.group(1))
        return None

    # --- Kolom omzetten naar uren ---
    df["charging_duration_hours"] = df["charging_duration"].apply(parse_duration_to_hours)

    # --- Verwijder rijen zonder geldige data ---
    ...
    st.markdown(
        """
        📊 *Deze grafiek toont de gemiddelde laadtijd per maand (in uren), automatisch omgerekend uit tekstuele waarden zoals "19 hours", "3 hours" of "an hour".*
        """
    )
